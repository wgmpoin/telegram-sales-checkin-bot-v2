...
worksheet = None
authorized_users_worksheet = None

async def initialize_google_sheets():
    """Menginisialisasi koneksi Google Sheets."""
    global worksheet, authorized_users_worksheet
    
    logger.info("Memulai inisialisasi Google Sheets...")
    encoded = os.getenv("GOOGLE_SERVICE_ACCOUNT")
    if not encoded:
        logger.error("GOOGLE_SERVICE_ACCOUNT belum diatur di environment.")
        raise ValueError("GOOGLE_SERVICE_ACCOUNT belum diatur di environment")
    ...
    logger.info(f"Mencoba membuka worksheet data utama: {TAB_NAME}...")
    try:
        worksheet = spreadsheet.worksheet(TAB_NAME)
        logger.info(f"Worksheet '{TAB_NAME}' berhasil dibuka.")
    except Exception as e:
        logger.error(f"Error saat membuka worksheet '{TAB_NAME}': {e}")
        raise

    logger.info(f"Mencoba membuka worksheet pengguna yang diotorisasi: {AUTHORIZED_USERS_TAB_NAME}...")
    try:
        authorized_users_worksheet = spreadsheet.worksheet(AUTHORIZED_USERS_TAB_NAME)
...
async def get_authorized_sales_list():
    ...
    if authorized_users_worksheet is None:
        logger.error("authorized_users_worksheet belum diinisialisasi. Tidak dapat mengambil daftar sales.")
        return []

    try:
        all_values = authorized_users_worksheet.get_all_values()
        if not all_values: 
            logger.warning("Sheet AUTHORIZED_USERS kosong atau tidak dapat diakses.")
            return []
...
async def receive_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Maps_link = f"https://www.google.com/maps/search/?api=1&query=-8.6753,115.2127{lat_rounded},{lon_rounded}"
    
    now = datetime.now()
    datetime_str = now.strftime("%Y-%m-%d %H:%M:%S") 
    
    if worksheet is None:
        logger.error("Worksheet utama belum diinisialisasi. Tidak dapat menulis data.")
...
async def main():
    ...
    await application.run_polling(poll_interval=1.0, timeout=10) # Menambahkan timeout dan interval polling
